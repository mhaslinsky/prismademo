    ...
    <>
      <div className={twMerge("flex items-center m-auto", smallMaxWith, className)}>
        <div className='flex-shrink-0 min-w-2xl'>
          {user?.image && <Image className='h-12 w-12 rounded-full' src={user.image} width={50} height={50} alt='' />}
        </div>
          ...
          <pre className='mt-5 mx-5 whitespace-pre-wrap break-words'>
            {post.language ? (
              <code
                ...
                dangerouslySetInnerHTML={{ __html: highlight(post.code, post.language) }}
              ></code>
            ) : (
              <code>{post.code}</code>
            )}
